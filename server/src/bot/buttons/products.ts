...
const button: Exported<[current: number, type: PaginateType]> = new Button()
	.setName('products')
	.listen(async(ctx, current: number, type: PaginateType) => {
		const [ page, total, data ] = await ctx.paginate(current, type, ctx.database.select({
			count: count(ctx.database.schema.products.id)
		}).from(ctx.database.schema.products).then((r) => r[0].count), ({ skip, take }) => ctx.database.select({
		...
		return ctx.interaction.update({
			...
			], components: ctx.paginateButtons(page, total, (type) => button(ctx.interaction, page, type), 1)
		})
	})
	.export()
